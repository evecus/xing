...
public final class Form32s extends InsnFormat {
    ...
    public boolean isCompatible(DalvInsn dalvInsn) {
        ...
        RegisterSpecList registers = dalvInsn.getRegisters();
        if (!(dalvInsn instanceof CstInsn) || registers.size() != 2 || !a.g(registers, 0) || !a.g(registers, 1)) {
            return false;
        }
    ...
    public void writeTo(AnnotatedOutput annotatedOutput, DalvInsn dalvInsn) {
        RegisterSpecList registers = dalvInsn.getRegisters();
        InsnFormat.write(annotatedOutput, InsnFormat.opcodeUnit(dalvInsn), InsnFormat.codeUnit(registers.get(0).getReg(), registers.get(1).getReg()), (short) ((CstLiteralBits) ((CstInsn) dalvInsn).getConstant()).getIntBits());
    }
}
