...
public final class Form41c extends InsnFormat {
    ...
    public boolean isCompatible(DalvInsn dalvInsn) {
        ...
        if (!InsnFormat.unsignedFitsInShort(registerSpec.getReg())) {
            return false;
        }
    ...
    public void writeTo(AnnotatedOutput annotatedOutput, DalvInsn dalvInsn) {
        RegisterSpecList registers = dalvInsn.getRegisters();
        InsnFormat.write(annotatedOutput, InsnFormat.opcodeUnit(dalvInsn), ((CstInsn) dalvInsn).getIndex(), (short) registers.get(0).getReg());
    }
}
