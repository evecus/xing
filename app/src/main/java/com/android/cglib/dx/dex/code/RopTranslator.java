...
public final class RopTranslator {
    ...
    public class LocalVariableAwareTranslationVisitor extends TranslationVisitor {
        ...
        public final RopTranslator this$0;
        ...
        @Override // com.android.cglib.dx.dex.code.RopTranslator.TranslationVisitor, com.android.cglib.dx.rop.code.Insn.Visitor
    ...
    public class TranslationVisitor implements Insn.Visitor {
        ...
        public void visitPlainCstInsn(PlainCstInsn plainCstInsn) {
            ...
            if (opcode2 != 3) {
                cstInsn = new CstInsn(dopDopFor, position, RopTranslator.getRegs(plainCstInsn), plainCstInsn.getConstant());
            } else {
                if (this.this$0.paramsAreInOrder) {
                    return;
                }
                RegisterSpec result = plainCstInsn.getResult();
                cstInsn = new SimpleInsn(dopDopFor, position, RegisterSpecList.make(result, RegisterSpec.make(((CstInteger) plainCstInsn.getConstant()).getValue() + (this.this$0.regCount - this.this$0.paramSize), result.getType())));
            }
            addOutput(cstInsn);
        }
        ...
        public void visitPlainInsn(PlainInsn plainInsn) {
            ...
            if (branchingness == 1 || branchingness == 2) {
                ...
                if (branchingness != 4) {
                    ...
                    simpleInsn = new TargetInsn(dopDopFor, position, RopTranslator.getRegs(plainInsn), this.this$0.addresses.getStart(this.block.getSuccessors().get(1)));
                }
            }
        ...
        public void visitSwitchInsn(SwitchInsn switchInsn) {
            ...
            CodeAddress codeAddress = new CodeAddress(position);
            SwitchData switchData = new SwitchData(position, this.lastAddress, cases, codeAddressArr);
        ...
        public void visitThrowingCstInsn(ThrowingCstInsn throwingCstInsn) {
            ...
            if ((dopDopFor.hasResult() || opcode.getOpcode() == 43) == (nextMoveResultPseudo != null)) {
                addOutput((opcode.getOpcode() != 41 || dopDopFor.getOpcode() == 35) ? new CstInsn(dopDopFor, position, regs, constant) : new SimpleInsn(dopDopFor, position, regs));
                return;
            }
    ...
    private RopTranslator(RopMethod ropMethod, int i, LocalVariableInfo localVariableInfo, int i2, DexOptions dexOptions) {
        ...
        int regCount = blocks.getRegCount() + (this.paramsAreInOrder ? 0 : i2);
        this.regCount = regCount;
        OutputCollector outputCollector = new OutputCollector(dexOptions, instructionCount, i3, regCount);
        this.output = outputCollector;
        this.translationVisitor = localVariableInfo != null ? new LocalVariableAwareTranslationVisitor(this, outputCollector, localVariableInfo) : new TranslationVisitor(this, outputCollector);
    }

    private static boolean calculateParamsAreInOrder(RopMethod ropMethod, int i) {
        boolean[] zArr = {true};
        ropMethod.getBlocks().forEachInsn(new Insn.BaseVisitor(zArr, ropMethod.getBlocks().getRegCount(), i) { // from class: com.android.cglib.dx.dex.code.RopTranslator.1
            public final int val$initialRegCount;
            public final int val$paramSize;
            public final boolean[] val$paramsAreInOrder;

            {
                this.val$paramsAreInOrder = zArr;
                this.val$initialRegCount = i;
                this.val$paramSize = i;
            }

            @Override // com.android.cglib.dx.rop.code.Insn.BaseVisitor, com.android.cglib.dx.rop.code.Insn.Visitor
            public void visitPlainCstInsn(PlainCstInsn plainCstInsn) {
                if (plainCstInsn.getOpcode().getOpcode() == 3) {
                    int value = ((CstInteger) plainCstInsn.getConstant()).getValue();
                    boolean[] zArr2 = this.val$paramsAreInOrder;
                    zArr2[0] = zArr2[0] && value + (this.val$initialRegCount - this.val$paramSize) == plainCstInsn.getResult().getReg();
                }
            }
        });
        return zArr[0];
